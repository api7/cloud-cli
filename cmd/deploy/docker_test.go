...
func TestDockerDeployCommand(t *testing.T) {
	testCases := []struct {
	...
	}{
		{
			...
			cmdPattern: `docker run --detach --mount type=bind,source=/.+?/\.api7cloud/cloud_lua_module,target=/cloud_lua_module,readonly --mount type=bind,source=/.+?/\.api7cloud/tls,target=/cloud/tls,readonly -p 9080:9080 -p 9443:9443 --name apisix --hostname apisix apache/apisix:2.11.0-centos`,
			mockCloud: func(t *testing.T) {
